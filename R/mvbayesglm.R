...
#' @useDynLib glmPOS
#' 
#' @importFrom stats coef cor glm model.matrix
#'
...
#' @param data0 \emph{Optional} historical data set for power prior on \eqn{\beta, \phi}
...
#' @param Gamma Initial value for correlation matrix. If \code{NULL} defaults to the correlation matrix from the responses.
...
mvbayesglm <- function(
  ...
  ## If data0 != NULL, do the same for historical data.
  ## 
  ...
  if( !is.null( data0 ) ) {
    y0mat <- sapply(formula.list, get_depvar, data0)
    Xlist <- lapply(formula.list, get_desmat, data0)
  ...
  smpl <- sample_copula_cpp ( 
    ...
    alpha0, gamma0, Gamma, v0, V0, b0, y0mat, X0list, M, beta0, phi0
  )
  ...
  c( smpl, 'formula.list' = formula.list )
}
